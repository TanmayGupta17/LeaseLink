...
<body>
    ...
    <div class="container">
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
            ...
            // Check if form is submitted with rating and location parameters
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['rate']) && isset($_POST['city'])) {
                // Retrieve rating and location input
                $rating = $_POST['rate'];
                $location = $_POST['city'];

                // Prepare and execute SQL statement to fetch properties and calculate average rating
                $sql = "SELECT property.*, AVG(rating.rate) AS avg_rating FROM property LEFT JOIN rating ON property.id = rating.propertyid WHERE property.address = ? GROUP BY property.id HAVING avg_rating = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sd", $location, $rating);
                $stmt->execute();
                $result = $stmt->get_result();

                // Display properties with average rating equal to the input rating and in the specified location
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<p class="card-text">Average Rating: ' . number_format($row['avg_rating'], 1) . '</p>'; // Display average rating
                    ...
                    echo "No properties found with an average rating of $rating in $location.";
                }
