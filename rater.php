...
<body>
    ...
    <div class="container">
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
            ...
            // Check if form is submitted with rating parameter
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['rate'])) {
                // Retrieve rating input
                $rating = $_POST['rate'];

                // Prepare and execute SQL statement to fetch properties and calculate average rating
                $sql = "SELECT property.*, AVG(rating.rate) AS avg_rating FROM property LEFT JOIN rating ON property.id = rating.propertyid GROUP BY property.id HAVING avg_rating = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("d", $rating);
                $stmt->execute();
                $result = $stmt->get_result();

                // Display properties with average rating equal to the input rating
                if ($result->num_rows > 0) {
                    ...
                    echo "No properties found with an average rating of $rating.";
                }
