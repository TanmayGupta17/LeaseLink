...
<body>
    ...
    <div class="container">
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
            ...
            // Check if form is submitted with BHK parameter
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['bhk'])) {
                // Retrieve BHK input
                $bhk = $_POST['bhk'];

                // Prepare and execute SQL statement to fetch properties matching the specified BHK
                $stmt = $conn->prepare("SELECT * FROM property WHERE bhk = ?");
                $stmt->bind_param("i", $bhk);
                $stmt->execute();
                $result = $stmt->get_result();

                // Display properties matching the specified BHK
                if ($result->num_rows > 0) {
                    ...
                    echo "No properties found with $bhk BHK.";
                }
