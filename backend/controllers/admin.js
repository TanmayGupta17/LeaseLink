...
const GetReportedProperties = async (req, res) => {
  try {
    const reportedProperties = await property.find({ reported: true });
    if (!reportedProperties || reportedProperties.length === 0) {
      return res.status(404).json({ message: "No reported properties found" });
    }
    res
      .status(200)
      .json({ message: "Reported Properties", reportedProperties });
  } catch (error) {
    console.log("Error while fetching reported properties", error);
    res
      .status(500)
      .json({ message: "Error while fetching reported properties" });
  }
};
...
const VerifyProperty = async (req, res) => {
  ...
  try {
    const propertyToVerify = await property.findById(id);
    if (!propertyToVerify) {
      return res.status(404).json({ message: "Property not found" });
    }
    propertyToVerify.Verification = "verified";
    await propertyToVerify.save();
    res.status(200).json({
      message: "Property verified successfully",
      property: propertyToVerify,
    });
  } catch (error) {
    console.log("Error while verifying property", error);
    res.status(500).json({ message: "Error while verifying property" });
  }
};
