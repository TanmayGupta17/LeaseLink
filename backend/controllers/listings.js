...
const CreateProperty = async (req, res) => {
  try {
    ...
    if (
      ...
      !leaseDuration ||
      !address ||
      furnished === undefined // Check if furnished is provided
    ) {
    ...
    if (!Array.isArray(amenitiesArray) || amenitiesArray.length === 0) {
      return res
        .status(400)
        .json({ message: "Amenities must be a non-empty array." });
    }
    ...
    //     message: "Invalid date format for leaseStartDate or leaseEndDate.",
    //   });
    // }
    ...
    //   return res
    //     .status(400)
    //     .json({ message: "leaseEndDate must be after leaseStartDate." });
    // }
    ...
    const property = await Property.create({
      ...
      leaseDuration,
      address,
      furnished, // Use the provided furnished value
      reported: false, // Default value for reported
      VerificationStatus: "UnVerified", // Default value for verification status
...
const GetUserProperties = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "User properties found", properties });
  } catch (error) {
