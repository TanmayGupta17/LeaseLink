...
const fetchActivityLogsForUser = async (userId) => {
  ...
  const handleEditUser = async () => {
    ...
    try {
      setLoading(true);
      const token = localStorage.getItem("token");
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/admin/users/${selectedUser._id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
                        ...
                        <div className="bg-gray-50 p-4 rounded-md">
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between">
                              ...
                              <span>{selectedUser?.properties?.length || 0}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-600">Active Properties:</span>
                              <span>{selectedUser?.properties?.filter(p => p.status === 'Active').length || 0}</span>
                            </div>
                          </div>
                        </div>
                      </div>
