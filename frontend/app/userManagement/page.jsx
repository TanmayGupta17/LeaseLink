...
const Toast = ({ message, type, onClose }) => {
  return (
    <div
      className={`fixed bottom-4 right-4 flex items-center p-4 rounded-md shadow-lg ${
        type === "success"
...
export default function AdminDashboard() {
  ...
  const fetchPropertiesForUser = async (userId) => {
    try {
      ...
      const response = await fetch(
        `${
          process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"
        }/admin/properties?userId=${userId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchActivityLogsForUser = async (userId) => {
    try {
      ...
      console.log(
        ...
        `${
          process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"
        }/admin/activitylog?userId=${userId}`
      ); // Debugging
  ...
  const handleEditUser = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${
          process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"
        }/admin/UpdateUser/${selectedUser._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const response = await fetch(
        `${
          process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"
        }/admin/deleteUser/${selectedUser._id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleEditProperty = async () => {
    if (!editPropertyData || !selectedProperty) {
      showToast("Missing property data", "error");
      return;
    }
    ...
    try {
      ...
      const propertyUpdateData = {
        title: editPropertyData.title || editPropertyData.name, // Support both field names
        location: editPropertyData.location || editPropertyData.address,
        price: editPropertyData.price,
        type: editPropertyData.type,
        status: editPropertyData.status,
      };

      console.log("Sending property update:", propertyUpdateData);

      const response = await fetch(
        `${
          process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"
        }/admin/UpdateUserProperty/${selectedProperty._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      // Check response status and parse error messages
      if (!response.ok) {
        const errorData = await response.text();
        console.error("Server response:", errorData);
        throw new Error(
          `HTTP ${response.status}: ${errorData || "Failed to update property"}`
        );
      }
      ...
      // Update local state with the corrected field names
      ...
      showToast(
        `Property "${propertyUpdateData.title}" has been updated successfully`
      );
    } catch (error) {
  ...
  const handleDeleteProperty = async () => {
    try {
      ...
      const response = await fetch(
        `${
          process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"
        }/admin/properties/${selectedProperty._id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      // Update local state after successful API call
      const updatedProperties = selectedUser.properties.filter(
      ...
      setIsPropertyModalOpen(false);
      showToast(
        `Property "${selectedProperty.name}" has been deleted`,
        "success"
      );
    } catch (error) {
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div
                className={`bg-white rounded-lg shadow p-6 ${
                  selectedUser ? "w-full md:w-1/3" : "w-full"
                }`}
              >
                    ...
                    filteredUsers.map((user) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center">
                            <span
                              className={`text-xs px-2.5 py-0.5 rounded-full ${getRoleBadgeColor(
                                user.role
                              )}`}
                            >
                              {user.role || "No role"}
                      ...
                      <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="bg-gray-50 p-4 rounded-md">
                            ...
                            <span
                              className={`text-sm px-2.5 py-1 rounded-full ${getRoleBadgeColor(
                                selectedUser.role
                              )}`}
                            >
                          ...
                          <div className="bg-gray-50 p-4 rounded-md">
                            ...
                            <span
                              className={`text-sm px-2.5 py-1 rounded-full ${getStatusBadgeColor(
                                selectedUser.status
                              )}`}
                            >
                        ...
                        <div className="bg-gray-50 p-4 rounded-md">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex justify-between">
                              ...
                              <span className="text-gray-700">
                                {selectedUser?.properties?.filter(
                                  (p) => p.status === "Active"
                                ).length || 0}
                              </span>
                            </div>
                                ...
                                <div className="flex justify-between items-start">
                                  <div>
                                    ...
                                    <div className="flex items-center mt-2">
                                      ...
                                      <span
                                        className={`ml-2 text-xs px-2 py-0.5 rounded-full ${getPropertyStatusColor(
                                          property.status
                                        )}`}
                                      >
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">
                This action cannot be undone. All properties and data associated
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditPropertyData({
                    ...editPropertyData,
                    title: e.target.value, // Changed from 'name' to 'title'
                  })
                }
                className="w-full px-3 py-2 border rounded-md text-gray-700"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditPropertyData({
                    ...editPropertyData,
                    location: e.target.value, // Changed from 'address' to 'location'
                  })
                }
                className="w-full px-3 py-2 border rounded-md text-gray-700"
                placeholder={`${selectedProperty?.location || "Enter Address"}`}
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 p-3 rounded-md">
              ...
              <div className="flex mt-2">
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full ${getPropertyStatusColor(
