        ...
        <div class="album_py-5_bg-body-tertiary">
            <div class="container">
                <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                    ...
                    // Check if form is submitted with price range
                    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['min']) && isset($_POST['max'])) {
                        // Retrieve price range input
                        $min_price = $_POST['min'];
                        $max_price = $_POST['max'];
        
                        // Prepare and execute SQL statement to fetch properties within the price range
                        $stmt = $conn->prepare("SELECT * FROM property WHERE price BETWEEN ? AND ?");
                        $stmt->bind_param("ii", $min_price, $max_price);
                        $stmt->execute();
                        $result = $stmt->get_result();
        
                        // Display properties within the price range
                        if ($result->num_rows > 0) {
                            ...
                            echo "No properties found within the specified price range.";
                        }
