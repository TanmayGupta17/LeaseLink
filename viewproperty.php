        ...
        <div class="album_py-5_bg-body-tertiary">
            <div class="container">
                <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                    ...
                    // Check if seller ID is provided via GET request
                    if (isset($_POST['username'])) {
                        // Retrieve seller ID
                        $seller_id = $_POST['username'];

                        // Prepare and execute SQL statement to fetch properties of the seller
                        $sql = "SELECT * FROM property WHERE username = ?";
                        $stmt = $conn->prepare($sql);
                        $stmt->bind_param("s", $seller_id);
                        $stmt->execute();
                        $result = $stmt->get_result();

                        // Display properties of the seller
                        if ($result->num_rows > 0) {
                            ...
                            echo "No properties found for this seller.";
                        }
