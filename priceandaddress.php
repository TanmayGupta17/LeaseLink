...
<body>
    ...
    <div class="container">
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
            ...
            // Check if form is submitted with price and BHK parameters
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['min']) && isset($_POST['bhk'])) {
                // Retrieve price and BHK input
                $min_price = $_POST['min'];
                // $max_price = $_POST['max_price'];
                $bhk = $_POST['bhk'];

                // Prepare and execute SQL statement to fetch properties within the price range and with the specified BHK
                $sql = "SELECT * FROM property WHERE price >= ? AND bhk = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("dd", $min_price, $bhk);
                $stmt->execute();
                $result = $stmt->get_result();

                // Display properties within the price range and with the specified BHK
                if ($result->num_rows > 0) {
                    ...
                    echo "No properties found within the specified price range and BHK.";
                }
